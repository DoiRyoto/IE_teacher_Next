...
test("入力フォームに'test'と入力し、検索ボタンを押すと'/search?query=test'に遷移する'", async ({ page }) => {
  await page.goto(url('/'));

  await page.getByRole("textbox", { name: "検索バー" }).fill("test");
  await page.getByRole("button", {name: "検索ボタン"}).click();
  await page.waitForTimeout(2000);
  await expect(page).toHaveURL(url("/search?query=test"));
});
test("入力フォームに'test'と入力し、'enterキー'を押すと'/search?query=test'に遷移する'", async ({ page }) => {
  await page.goto(url('/'));
